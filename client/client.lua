...
CreateThread(function()
        ...
        options = {
            {
                ...
                action = function(entity) 
                    ...
                    TriggerEvent("kael-cigar:client:PickUpTobacco", entity)
                end,
            },
    ...
    exports[Config.Target]:AddBoxZone('Shop', vector3(26.65, -1108.14, 29.48), 1.6, 1, {
        ...
        options = {
            {  
                event = "kael-cigar:server:OpenShop",
                icon = "fas fa-shop",
                label = "Access Shop",
            },
        },
    ...
    maxZ=29.48
        ...
        options = {
            {  
                event = "kael-cigar:client:OpenMenu",
        ...
        options = {
            {  
                ...
                canInteract = function(entity, distance, data)
                    local isboss = QBCore.Functions.GetPlayerData().job.isboss
                    if isboss then 
                        return true
                    end
                    return false
...
RegisterNetEvent("kael-cigar:client:PickUpTobacco", function(weedplant)
    ...
    QBCore.Functions.Progressbar("PickTobacco", "Picking Up Tobacco..", 5000, false, true, {
        ...
        TriggerServerEvent("kael-cigar:server:PickUpTobacco")
        DeleteEntity(weedplant)
        ClearPedTasks(PlayerPedId())
    end) 
end)

RegisterNetEvent("kael-cigar:server:OpenShop", function()
...
RegisterNetEvent("kael-cigar:client:OpenMenu", function()
    local MenuProducts = {
        { 
            header = 'Make Cigar', 
    ...
    for k, v in pairs(Config.Products) do
        MenuProducts[#MenuProducts + 1] = {
            ...
            icon = k,
            params = {
                event = 'kael-cigar:client:makingItems',
                args = {
                    item = k
                }
            }
...
RegisterNetEvent('kael-cigar:client:makingItems', function(data) 
    QBCore.Functions.TriggerCallback("kael-cigar:server:makingcigar", function(cb)
        if cb then 
            ...
            QBCore.Functions.Progressbar("makeitem", "Making " .. QBCore.Shared.Items[data.item].label, 4000, false, true, {
                ...
                TriggerServerEvent("kael-cigar:server:GiveCigar", data.item)
